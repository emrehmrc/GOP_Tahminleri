...
class HybridXGBModel:
    ...
    def get_booster(self):
        return self.model_wd_sat.get_booster()
...
class ModelManager:
    ...
    def train_model(self, X_train, y_train, X_test, y_test):
        ...
        if MAX_TRAIN_SIZE is not None and len(X_train) > MAX_TRAIN_SIZE:
            print(f"[ModelManager] Capping training set size to last {MAX_TRAIN_SIZE} samples (was {len(X_train)})")
        ...
        import json
        from config_live import HPO_PARAMS_SUFFIX, ENABLE_WEEKEND_SPLIT_XGB
        ...
        params = {
            ...
            'min_child_weight': 13,
            'reg_alpha': 8.924757905144489,
            'reg_lambda': 2.005839273119691,
        ...
        param_file_wd = os.path.join(project_root, f"best_params_xgb_general{suffix}.json")
        if os.path.exists(param_file_wd):
            print(f"[ModelManager] Loading optimized weekday parameters from: {param_file_wd}")
            try:
                with open(param_file_wd, "r") as f:
                    opt_params_wd = json.load(f)
                params_wd.update(opt_params_wd)
            except Exception as e:
                print(f"[ModelManager] UYARI: Weekday parametre dosyası okunamadı: {e}")
        else:
            print(f"[ModelManager] Weekday parametre dosyası bulunamadı ({param_file_wd}). Varsayılanlar kullanılacak.")
            
        # Load weekend parameters
        params_we = params_wd.copy()
        param_file_we = os.path.join(project_root, f"best_params_xgb_weekend{suffix}.json")
        if os.path.exists(param_file_we):
            print(f"[ModelManager] Loading optimized weekend parameters from: {param_file_we}")
            try:
                with open(param_file_we, "r") as f:
                    opt_params_we = json.load(f)
                params_we.update(opt_params_we)
            except Exception as e:
                print(f"[ModelManager] UYARI: Weekend parametre dosyası okunamadı: {e}")
        else:
            print(f"[ModelManager] Weekend parametre dosyası bulunamadı ({param_file_we}). Hafta içi parametreleri kullanılacak.")
            
        import config_live as config
        # Scale estimators and early stopping in full mode if learning rate is very low to allow convergence
        for p_dict in [params_wd, params_we]:
            if not getattr(config, 'FAST_MODE', False) and p_dict.get('learning_rate', 0.1) < 0.02:
                p_dict['n_estimators'] = max(p_dict.get('n_estimators', 1000), 3000)
                p_dict['early_stopping_rounds'] = max(p_dict.get('early_stopping_rounds', 50), 120)
        ...
        if getattr(config, 'FAST_MODE', False) and not os.environ.get('OE_FULL_STRENGTH'):
            ...
            print(f"[ModelManager] Hızlı mod aktif. n_estimators değeri {fast_max_iter} olarak güncelleniyor.")
            for p_dict in [params_wd, params_we]:
                p_dict['n_estimators'] = fast_max_iter
                p_dict['max_depth'] = min(5, p_dict.get('max_depth', 5))
                p_dict['early_stopping_rounds'] = min(15, p_dict.get('early_stopping_rounds', 50))
                
                # Scale learning rate to prevent underfitting in fast mode when tree count is severely limited
        ...
        if ENABLE_WEEKEND_SPLIT_XGB and dow_train is not None:
            print("[ModelManager] Hybrid Split Model training active (Mon-Sat vs Sat-Sun).")
            ...
            print(f"[ModelManager] Training Model A (WD+Sat) on {len(X_train_wd_sat)} samples...")
            model_wd_sat = self._fit_single_model(X_train_wd_sat, y_train_wd_sat, params_wd)
            
            print(f"[ModelManager] Training Model B (WE) on {len(X_train_we)} samples...")
            model_we = self._fit_single_model(X_train_we, y_train_we, params_we)
            
            self.model = HybridXGBModel(model_wd_sat, model_we)
            print("[ModelManager] Hybrid training finished successfully.")
        else:
            print(f"[ModelManager] Training standard single model on {len(X_train)} samples...")
    ...
    def _fit_single_model(self, X_train, y_train, params):
        ...
        if ENABLE_GBDT_REFIT and X_val is not None:
            ...
            if best_iter is None or best_iter <= 0:
                best_iter = params.get('n_estimators', 1000)
            
            print(f"[ModelManager] Erken durdurma ile en iyi iterasyon: {best_iter}. Model tüm veriyle (%100) yeniden eğitiliyor...")
    ...
    def save_model(self, filename=MODEL_NAME):
        ...
        if isinstance(self.model, HybridXGBModel):
            # Save both submodels
            wd_sat_path = save_path.replace('.json', '_wd_sat.json')
            we_path = save_path.replace('.json', '_we.json')
            ...
            with open(save_path, 'w', encoding='utf-8') as f:
                f.write("HYBRID_XGB_SPLIT\n")
                f.write(f"wd_sat_model: {os.path.basename(wd_sat_path)}\n")
                f.write(f"we_model: {os.path.basename(we_path)}\n")
            print(f"[ModelManager] Hybrid XGB models saved to: {wd_sat_path} and {we_path}")
        else:
        ...
        if os.path.exists(load_path):
            ...
            try:
                with open(load_path, 'r', encoding='utf-8') as f:
                    ...
                    if first_line == "HYBRID_XGB_SPLIT":
                        is_hybrid = True
            ...
            if is_hybrid:
                ...
                self.model = HybridXGBModel(model_wd_sat, model_we)
                print(f"[ModelManager] Hybrid XGB models loaded from pointer: {load_path}")
            else:
    ...
    def get_feature_importance(self, max_num=20):
        ...
        if isinstance(self.model, HybridXGBModel):
            print("[ModelManager] Hybrid Split: Showing feature importances from Model A (WD+Sat).")
            model_to_plot = self.model.model_wd_sat
        else:
            model_to_plot = self.model
            
        importance = model_to_plot.get_booster().get_score(importance_type='gain')
        ...
        xgb.plot_importance(model_to_plot, importance_type='gain', max_num_features=max_num, 
